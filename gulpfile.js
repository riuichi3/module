...
gulp.task("aigis", function() {
  gulp.src( '_aigis/aigis_config.yml' )
    .pipe(aigis());
});
...
gulp.task('replace', function(){ //「replace」のタスク内容を書こう
  gulp.src( dir.dist+'/common/css/style.css' ) //「style.css」ファイルを指定
...
gulp.task('sprite', function () {
  const spriteData = gulp.src( dir.src + '/common/images/sprite/*.png' ) //スプライトにする愉快な画像達
  .pipe(spritesmith({
...
gulp.task('pug', function() {
    return gulp.src([dir.src + '/{,**/}*.pug', '!./_src/**/_*.pug'])
        .pipe(plumber({
            errorHandler: notify.onError("Error: <%= error.message %>")
        }))
        .pipe(pug({
...
return gulp.src([dir.src + '/{,**/}js.pug', dir.src + '/{,**/}css.pug'])
    .pipe(plumber({
        errorHandler: notify.onError("Error: <%= error.message %>")
    }))
    .pipe(pug({
...
gulp.task('iconfonts', function(){
  ...
  return gulp.src(dir.src + '/common/css/fonts/*.svg')
    .pipe(iconfont({
      startUnicode: 0xF001,
      fontName: 'icon',
   ...
   .on('glyphs', function(glyphs) {
      gulp.src(dir.src + '/common/css/fonts/_icon.scss')
